...
class MLP:
    def __init__(self, weight_init_std=0.032, hidden_unit=200):
        ...
        self.dB = [weight_init_std * cp.random.randn(hidden_unit, hidden_unit), weight_init_std * cp.random.randn(hidden_unit, hidden_unit),
                   weight_init_std * cp.random.randn(hidden_unit, hidden_unit), weight_init_std * cp.random.randn(10, hidden_unit)]
    ...
    def feedback_alignment(self, x, target, batch_size, alpha):
        h1_ = cp.dot(x, self.W_f1)
        # h1_ = cp.tanh(h1)
        h2_ = cp.dot(h1_, self.W_f2)
        # h2_ = cp.tanh(h2)
        h3_ = cp.dot(h2_, self.W_f3)
        # h3_ = cp.tanh(h3)
        h4_ = cp.dot(h3_, self.W_f4)
        # h4_ = cp.tanh(h4)
        ...
        delta4 = cp.dot(delta5, self.dB[3])
        delta_Wf4 = cp.dot(h3_.T, delta4)
        delta3 = cp.dot(delta4, self.dB[2])
        delta_Wf3 = cp.dot(h2_.T, delta3)
        delta2 = cp.dot(delta3, self.dB[1])
        delta_Wf2 = cp.dot(h1_.T, delta2)
        delta1 = cp.dot(delta2, self.dB[0])
