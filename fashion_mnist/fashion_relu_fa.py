...
class MLP:
    ...
    def feedback_alignment(self, x, target, batch_size, alpha):
        ...
        delta3 = relu_grad(h3) * cp.dot(delta4, self.dB[2])
        delta_Wf3 = cp.dot(h2_.T, delta3)
        delta2 = relu_grad(h2) * cp.dot(delta3, self.dB[1])
        delta_Wf2 = cp.dot(h1_.T, delta2)
        delta1 = relu_grad(h1) * cp.dot(delta2, self.dB[0])
...
if __name__ == '__main__':
    ...
    cp.save('./weights_fa_relu_W_f1', mlp.W_f1)
    cp.save('./weights_fa_relu_W_f2', mlp.W_f2)
