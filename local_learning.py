...
for i in range(50000):
    ...
    if i % iter_per_epoch == 0:
        ...
        train_loss = mlp.loss(x_train, t_train)
        test_loss = mlp.loss(x_test, t_test)
        train_loss_list_l.append(cuda.to_cpu(train_loss))
        test_loss_list_l.append(cuda.to_cpu(test_loss))
        train_acc_list_l.append(cuda.to_cpu(train_acc))
        test_acc_list_l.append(cuda.to_cpu(test_acc))
        print("epoch:", int(i / iter_per_epoch), " train acc, test acc | " + str(train_acc) + ", " + str(test_acc))
...
plt.savefig("./result/0723/FA_for_mnist.png")
