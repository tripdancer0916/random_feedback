...
for i in range(100000):
    ...
    if i % iter_per_epoch == 0:
        ...
        global_error_variable = np.dot(cuda.to_cpu(mlp.d), cuda.to_cpu(mlp.predict(x_train)))
        compared_error_variable = np.dot(np.ones(10), cuda.to_cpu(mlp.predict(x_train)))
