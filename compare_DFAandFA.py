...
class MLP:
    def __init__(self, weight_init_std=0.01):
        ...
        self.B4fa = weight_init_std * cp.random.randn(10, hidden_unit)
        self.B3fa = weight_init_std * cp.random.randn(hidden_unit, hidden_unit)
        self.B2fa = weight_init_std * cp.random.randn(hidden_unit, hidden_unit)

        self.B4dfa = weight_init_std * cp.random.randn(10, hidden_unit)
    ...
    def feedback_alignment(self, x, target):
        ...
        delta3 = cp.dot(delta4, self.B4fa)
        delta_Wf3 = cp.dot(h2_.T, delta3)

        delta2 = cp.dot(delta3, self.B3fa)
        delta_Wf2 = cp.dot(h1_.T, relu_grad(h2) * delta2)

        delta1 = cp.dot(delta2, self.B2fa)
        delta_Wf1 = cp.dot(x.T, relu_grad(h1) * delta1)
    ...
    def direct_feedback_alignment(self, x, target):
        ...
        delta3 = cp.dot(delta4, self.B4dfa)
        delta_Wf3 = cp.dot(h2_.T, delta3)

        delta2 = cp.dot(delta4, self.B3dfa)
        delta_Wf2 = cp.dot(h1_.T, relu_grad(h2) * delta2)

        delta1 = cp.dot(delta4, self.B2dfa)
        delta_Wf1 = cp.dot(x.T, relu_grad(h1) * delta1)
...
for i in range(50000):
    ...
    if i % iter_per_epoch == 0:
        ...
        train_loss = mlp.loss(x_train, t_train)
        test_loss = mlp.loss(x_test, t_test)
        train_loss_list_DFA.append(cuda.to_cpu(train_loss))
        test_loss_list_DFA.append(cuda.to_cpu(test_loss))
        train_acc_list_DFA.append(cuda.to_cpu(train_acc))
...
plt.plot(test_acc_list_FA, label="FA", color="orange")

# plt.plot(train_acc_list_l, label="only last layer", linestyle="dashed", color="orange")
plt.plot(test_acc_list_DFA, label="DFA", color="green")
...
plt.savefig("./result/0708/compare_FAandDFA.png")
